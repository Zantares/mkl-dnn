...
#undef PARAM_OFF
    ...
    jit_softmax_base_t(const softmax_pd_t *pd)
        : pd_(pd), data_d_(pd_->is_fwd() ? pd_->src_md() : pd_->dst_md()) {
...
struct jit_softmax_t<avx512_common> : public jit_softmax_base_t<avx512_common> {
    ...
    void store(const Address &addr, const Vmm &vmm,
            const Opmask mask = Opmask(0)) {
        auto effective_addr = addr;
        if (mask != k0) effective_addr = addr | mask;
        if (is_bf16_) {
            if (bf16_emu_)
    ...
    void load(const Vmm &vmm, const Address &addr,
            const Opmask mask = Opmask(0)) {
        auto effective_vmm = vmm;
        if (mask != k0) effective_vmm = vmm | mask | T_z;
    ...
    void accumulate_vmax() override {
        ...
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                load(vreg_tmp_src, src_ptr(axis_stride_ * i),
                        tail ? tail_opmask : k0);
                if (tail)
    ...
    void compute_dst() override {
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                if (!tail) {
                    if (is_softmax_) {
                        load(vreg_tmp_src, dst_ptr(axis_stride_ * i));
                    ...
                    store(dst_ptr(axis_stride_ * i), vreg_tmp_src, tail_opmask);
                }
            }
        });
    }
    ...
    void accumulate_vsbr() override {
        ...
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                ...
                if (!tail) {
                    load(vreg_tmp_diff_dst, diff_dst_ptr(axis_stride_ * i));
                    if (is_softmax_) {
                        load(vreg_tmp_dst, dst_ptr(axis_stride_ * i));
                        vmulps(vreg_tmp_diff_dst, vreg_tmp_diff_dst,
                                vreg_tmp_dst);
                    }
                    vaddps(vsbr, vsbr, vreg_tmp_diff_dst);
                } else {
                    load(vreg_tmp_diff_dst, diff_dst_ptr(axis_stride_ * i),
                            tail_opmask);
                    if (is_softmax_) {
                        load(vreg_tmp_dst, dst_ptr(axis_stride_ * i),
                                tail_opmask);
                        vmulps(vreg_tmp_diff_dst | tail_opmask,
                                vreg_tmp_diff_dst, vreg_tmp_dst);
                    }
                    vaddps(vsbr | tail_opmask, vsbr, vreg_tmp_diff_dst);
                }
            }
        });
    ...
    void compute_diff_src() override {
        axis_loop([&](int unroll, bool tail = false) {
            for (int i = 0; i < unroll; i++) {
                Vmm vreg_tmp_dst = Vmm(i * 2 + 1);
                Vmm vreg_tmp_diff_dst = Vmm(i * 2 + 2);
                auto effective_mask = tail ? tail_opmask : k0;
                load(vreg_tmp_dst, dst_ptr(axis_stride_ * i), effective_mask);
                load(vreg_tmp_diff_dst, diff_dst_ptr(axis_stride_ * i),
                        effective_mask);
                if (is_softmax_) {
                ...
                if (is_logsoftmax_) {
                    ...
                    vmulps(vreg_tmp_dst, vreg_tmp_dst, vsbr);
                    vsubps(vreg_tmp_diff_dst, vreg_tmp_diff_dst, vreg_tmp_dst);
                }
                store(diff_src_ptr(axis_stride_ * i), vreg_tmp_diff_dst,
                        effective_mask);
            }
        });
    }
